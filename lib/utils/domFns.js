...
export function offsetXYFromParent(evt: {clientX: number, clientY: number}, offsetParent: HTMLElement, scale: number): ControlPosition {
  ...
  const x = parseInt((evt.clientX + offsetParent.scrollLeft - offsetParentRect.left) / scale);
  const y = parseInt((evt.clientY + offsetParent.scrollTop - offsetParentRect.top) / scale);
